                ...
                <div class="row">
                    <div class="col-12 col-md-12 col-lg-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="{{ route('opdbps.update', $bps->id) }}" runat="server" method="post">
                                    @csrf
                                    @method('patch')
                                    <div class="form-group">
                                        ...
                                        <select name="kategori" id="kategori"
                                            ...
                                            @endphp
                                            @foreach ($kategori['data'][1] as $kategori_item)
                                                <option value="{{ $kategori_item['title'] }}"
                                                    {{ $bps->kategori == $kategori_item['title'] ? 'selected' : '' }}>
                                                    {{ $kategori_item['title'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <select name="sub_kategori" id="sub-kategori"
                                            ...
                                            @foreach ($demografi as $item)
                                                <option value="{{ $item['title'] }}"
                                                    {{ $bps->sub_kategori == $item['title'] ? 'selected' : '' }}>
                                                    {{ $item['title'] }}
                                                </option>
                                            @endforeach
