...
@section('main')
            ...
            <div class="section-body">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-12 col-lg-12">
                        <form action="{{ route('datasets.update', $datasets->id) }}" id="main-form"
                            enctype="multipart/form-data" runat="server" method="post">
                            @csrf
                            @method('patch')
                            ...
                            <fieldset {{ $isReadOnly ? 'disabled' : '' }}>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            <label>Status</label>
                                            <select name="status" onchange="validate_status(this);"
                                                class="form-control @error('status')is-invalid @enderror select2 ">
                                                <option disabled>=== Pilih Salah Satu ===</option>
                                                <option value="PENDING"
                                                    {{ $datasets->status == 'PENDING' ? 'selected' : '' }}>
                                                    Dalam Proses</option>
                                                <option value="REPAIR"
                                                    {{ $datasets->status == 'REPAIR' ? 'selected' : '' }}>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div id="metadatahelp" class="form-text">Upload Pdf: Ms-Kegiatan, Ms-Variabel
                                                ...
                                                Indikator (dalam 1 file .Pdf). <a
                                                    href="{{ asset('assets/metadata/' . $datasets->metadata ?? '-') }}">Lihat
                                                    File
                                                    Sebelumnya</a> </div>
                                        </div>
                                        @if (!$isReadOnly)
    ...
    @if ($isReadOnly)
        <script>
            // Mencari semua input, textarea, dan select di dalam form
            const form = document.getElementById('main-form');
            const elements = form.querySelectorAll('input, textarea, select');

            elements.forEach(el => {
                el.readOnly = true; // Untuk input teks
                if (el.tagName === 'SELECT' || el.type === 'checkbox' || el.type === 'radio') {
                    el.disabled = true; // Select & Checkbox harus pakai disabled
                }
            });

            $(document).ready(function() {
                // Menonaktifkan editor Summernote
                $('.summernote').summernote('disable');

                // Opsional: Mengubah warna background agar terlihat seperti readonly
                $('.note-editable').css('background-color', '#e9ecef');

                $('input[data-role="tagsinput"]').tagsinput('destroy'); // Hapus fungsi editnya

                $('input[data-role="tagsinput"]').attr('readonly', true); // Jadikan readonly

                // 2. Jika Anda ingin tag tetap terlihat tapi tidak bisa dihapus:
                // Plugin ini biasanya membungkus tag dalam class .bootstrap-tagsinput
                $('.bootstrap-tagsinput input').prop('disabled', true);
                $('.bootstrap-tagsinput .tag [data-role="remove"]').hide(); // Sembunyikan tombol 'x'

                // 3. Tambahan untuk elemen lain yang Anda punya
                $('.summernote').summernote('disable');
