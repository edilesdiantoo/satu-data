...
@section('main')
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a class="dropdown-item"
                                                        href="{{ route('web-agenda.download', [$agenda->id, Str::slug($agenda->judul_rilis)]) }}">
                                                        Excel
                                                    </a>
                                                </li>
                                                <li>
                                                    <a class="dropdown-item"
                                                        href="{{ route('web-agenda.downloadCsv', [$agenda->id, Str::slug($agenda->judul_rilis)]) }}">
                                                        CSV
                                                    </a>
                                                </li>
                                            </ul>
                                                        ...
                                                        <thead class="text-white text-lowercase"
                                                            ...
                                                            <tr>
                                                                ...
                                                                @foreach ($table as $column)
                                                                    {{-- Lewati kolom ID asli tabel agar tidak double dengan penomoran baru --}}
                                                                    @if ($column == 'id')
                                                                        @continue
                                                                    @endif
    ...
    <script>
        ...
        var dataArray = @json($maps_data);

        function popUp(feature, layer) {
            // Cari data yang nama_kabupaten-nya sama dengan properti di GeoJSON
            let info = dataArray.find(item =>
                item.nama_kabupaten.toUpperCase() === feature.properties.KAB_KOTA.toUpperCase()
            );

            if (feature.properties && feature.properties.KAB_KOTA) {
                let labelValue = {!! json_encode(app('request')->input('value_peta') ?? 'Data') !!};
                let displayValue = info ? info.value : 'Data tidak tersedia';
                ...
                <div class="popup-card">
                    ...
                    <div class="text-wrapper">
                        <p>
                            Data yang terdapat di Kota/Kabupaten ini: <br>
                            <b>${labelValue} : ${displayValue}</b>
                        </p>
                    </div>
                </div>`;
    ...
    <script>
        $(document).ready(function() {
            $('#table-5').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('web-agenda.fetch', ['id' => $agenda->id, 'bearer' => env('BEARER_TOKEN')]) }}",
                    type: 'GET'
                },
                columns: [
                    // 1. Nomor Urut
                        ...
                        @if ($column == 'kode_kabupaten_kota')
                            {
                                ...
                                searchable: false, // Tambahkan ini agar tidak error saat search
                                orderable: false,
                                defaultContent: '-'
                            },
                        @elseif ($column == 'kode_kecamatan') {
